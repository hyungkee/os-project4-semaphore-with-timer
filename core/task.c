...
/*
void print_queue(_os_node_t* head){
	_os_node_t* node = head;
	int cnt = 0;
	while(node != NULL){
		printf("%d's node in queue, task : %x, priority : %d\n", cnt++, node->ptr_data, ((eos_tcb_t*)node->ptr_data)->priority);
		node = node->next;
		if(node == head)
			break;
	}
...
void reposition_task(eos_tcb_t *task){
	...
	if(task->state == SEMAPHORE)	return;
